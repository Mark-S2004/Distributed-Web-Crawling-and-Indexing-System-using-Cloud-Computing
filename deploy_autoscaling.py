...
def deploy_autoscaling(stack_name, template_file, parameters):
    """
    Deploy the auto-scaling group using CloudFormation
    """
    try:
...
def main():
    parser = argparse.ArgumentParser(description='Deploy auto-scaling crawler nodes')
    parser.add_argument('--stack-name', default='web-crawler-autoscaling',
                        help='Name of the CloudFormation stack')
    parser.add_argument('--template', default='ec2_auto_scaling.yaml',
    ...
    parser.add_argument('--min-instances', type=int, default=1,
                        help='Minimum number of crawler instances')
    parser.add_argument('--max-instances', type=int, default=5,
                        help='Maximum number of crawler instances')
    parser.add_argument('--desired-capacity', type=int, default=2,
                        help='Desired number of crawler instances')
    ...
    parameters = {
        ...
        'MinInstances': str(args.min_instances),
        'MaxInstances': str(args.max_instances),
        'DesiredCapacity': str(args.desired_capacity),
    ...
    # Deploy the stack
    success = deploy_autoscaling(args.stack_name, args.template, parameters)
